...
type TopologyEdge = Record<string, unknown>;
...
export default async function HomePage() {
  ...
  const [topologyRes, incidentsRes] = await Promise.allSettled([
    fetchTopology(),
    fetchIncidents(),
  ]);

  if (topologyRes.status === "fulfilled") {
    topology = topologyRes.value;
  }
  ...
  const pendingRcaCount = incidents.filter(
    (i) => i.status === "active" && !i.rca
  ).length;

  const healthyNodes =
    topology.nodes?.filter((n) => n.status === "healthy").length ?? 0;
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <AICopilot />
            <RCAWarmup pendingCount={pendingRcaCount} />
          </div>
        </div>
      </div>
    </div>
